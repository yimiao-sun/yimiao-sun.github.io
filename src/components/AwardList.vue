...
<template>
  ...
  <div v-for="(awardList, listIdx) in awardArr" :key="listIdx">
    <div class="row items-align-top" v-for="(award, index) in awardList.list">
      <label style="font-size: 11pt" class="col" :for="`${idPrefix}-${listIdx}-${index}`" >{{ award.date }}</label>
      <div style="font-size: 11pt" class="col flex-start tight-list" :id="`${idPrefix}-${listIdx}-${index}`" v-html="award.content"></div>
    </div>
  </div>
</template>